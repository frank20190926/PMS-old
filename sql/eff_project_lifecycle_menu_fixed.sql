...
-- 获取人效中心的menu_id
SET @efficiency_center_id := (SELECT menu_id FROM sys_menu WHERE menu_name = '人效中心' LIMIT 1);
...
INSERT INTO `sys_menu` (
    ...
    @max_menu_id + 1, '项目生命周期', @efficiency_center_id,
    5, 'project', NULL, 1, 0, 'M', '0', '0', '',
    'el-icon-s-cooperation', NOW(), '项目全生命周期管理'
);
...
INSERT INTO `sys_menu` (
    ...
    'pms:efficiency:task-generation:list', 'el-icon-tickets', NOW(), '任务自动生成管理'
);
...
INSERT INTO `sys_menu` (
    ...
    'pms:efficiency:project:workflow:query', 'el-icon-data-line', NOW(), '项目流程状��跟踪'
);

-- ====================================================================
-- 6. 为PM角色（role_id=102/103/104）和管理员（role_id=2）分配权限
-- ====================================================================
...
-- SELECT * FROM sys_role_menu WHERE role_id IN (102, 103, 104, 2) AND menu_id >= @lifecycle_menu_id ORDER BY role_id;
-- SELECT COUNT(*) as total_menus FROM sys_menu WHERE menu_id >= @lifecycle_menu_id;
