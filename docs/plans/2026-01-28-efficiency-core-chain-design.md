...
### 2.13 验收口径
- **范围**: 覆盖核心链路 + 老系统回写联动。
- **对账**: 必须对账人效日报与老系统 `pms_report`，按项目拆分逐条对齐。
- **权限**: 抽样验证部门负责人只读、PM 管理、员工权限边界。
- **性能**: 可选，功能通过后补充性能指标。
- **端到端**: 必须验证完整流程：初始化/排期 → 下发 → 日报 → 审核 → 周报 → 回写。
- **通过标准**:
  1) 功能清单全部通过
  2) 数据对账一致
  3) 权限抽样通过

### 2.14 补充规则（冲突/工时/回写/模板/验收/可见）
- **排期冲突**: 提示预警并给出可安排时间段，PM 沟通确认后可继续。
- **日报工时**: 超过 8 小时允许提交但需理由；回写老系统固定 8 小时，超出仅保留在人效中心。
- **回写失败**: 提供失败列表、详情处理与手动重试，并通知 PM/管理员。
- **模板更新**: 系统提示引导更新，PM 可选择不更新已创建项目。
- **任务验收**: 任务完成需 PM 验收确认。
- **跨部门可见**: 由项目 PM 配置可见范围。
